...
public class PropertiesFileConvertAction extends AnAction {
  ...
  private PropertiesToYamlConverter propertiesToYamlConverter = new PropertiesToYamlConverter();
  ...
  public void actionPerformed(AnActionEvent event) {
    ...
    try {
      ...
      Messages.showMessageDialog(project, "Unable to create Yaml file.", "Error",
          Messages.getInformationIcon());
    }
  }

  private void openYamlFile(Project project, String yamlFilePath) {
    VirtualFile yamlVirtualFile = LocalFileSystem.getInstance()
        .refreshAndFindFileByIoFile(new File(yamlFilePath));
  ...
  private String createYamlFile(EditorImpl editor, String str) {
    ...
    String yamlFilePath = virtualFile.getPath().substring(0,
        virtualFile.getPath().length() - PluginConstants.PROPERTIES_FILE_EXTENSION.length())
        + "yml";
  ...
  private String getYamlString(AnActionEvent event) {
    ...
    return propertiesToYamlConverter.toYamlString(propertiesFilePath);
  }
  ...
  private boolean isVisible(Editor editor) {
    return editor != null && PluginConstants.PROPERTIES_FILE_TYPE
        .equals(((EditorImpl) editor).getVirtualFile().getFileType());
  }
}